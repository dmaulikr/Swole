...
//  Pods
//
//  Created by Wenchao Ding on 12/3/15, Victor Lin 7/5/15
//
//
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //edited
        UIButton *cellButton = [[UIButton alloc] initWithFrame:CGRectZero];
        cellButton.enabled = NO;
        [cellButton addTarget:self.entryListViewControllerDelegate action:@selector(doCellButton) forControlEvents:UIControlEventTouchDown];
        [self.contentView addSubview: cellButton];
        self.dateButton = cellButton;
...
- (void)setBounds:(CGRect)bounds
{
    ...
    _eventLayer.frame = CGRectMake((_backgroundLayer.frame.size.width-eventSize)/2+_backgroundLayer.frame.origin.x + 1, CGRectGetMaxY(_backgroundLayer.frame) - eventSize*1.5f, eventSize*0.8, eventSize*0.8); //edited
...
- (void)configureCell
{
    ...
    NSDictionary *fontAttributes =   [NSDictionary dictionaryWithObjectsAndKeys:
                                      _appearance.titleFont,
                                      NSFontAttributeName, nil];
    [titleString setAttributes:fontAttributes range:NSMakeRange(0, [titleString length])];
    [_dateButton setAttributedTitle: titleString forState: UIControlStateNormal];
    [_dateButton setTitleColor:[self colorForCurrentStateInDictionary:_appearance.titleColors] forState: UIControlStateNormal];
    if (self.placeholder) {
        _dateButton.titleLabel.alpha = .2f;
    } else {
        _dateButton.titleLabel.alpha = 1.f;
    }
    ...
    CGFloat titleHeight = [_dateButton.titleLabel.text sizeWithAttributes:@{NSFontAttributeName:self.dateButton.titleLabel.font}].height;
    if (_subtitleLabel.text) {
        ...
        _dateButton.frame = CGRectMake(0,
                                       (self.contentView.fs_height*5.0/6.0-height)*0.5,
                                       self.fs_width,
                                       titleHeight);
        
        _subtitleLabel.frame = CGRectMake(0,
                                          _dateButton.titleLabel.fs_bottom - (_dateButton.titleLabel.fs_height-_dateButton.titleLabel.font.pointSize),
                                          self.fs_width,
                                          subtitleHeight);
    } else {
        _dateButton.frame = CGRectMake(0, 0, self.fs_width, floor(self.contentView.fs_height*5.0/6.0));
    ...
    if (self.isSelected) {
        ...
        [self.entryListViewControllerDelegate animateEntryListViewLabels];
    } else {
