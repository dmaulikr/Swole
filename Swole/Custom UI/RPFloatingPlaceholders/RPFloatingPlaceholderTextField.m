...
- (void)setPlaceholder:(NSString *)aPlaceholder
{
    ...
    self.floatingLabel.textAlignment = NSTextAlignmentCenter; //adjust code to center the floating placeholder
...
- (void)setupViewDefaults
{
    ...
    self.floatingLabel.font = [UIFont fontWithName:@"AvenirNext-Regular" size:14.5f];
...
- (void)setupDefaultColorStates {
    ...
    defaultActiveColor = [UIColor ht_bitterSweetColor];
...
- (void)drawRect:(CGRect)aRect
{
    ...
    if (self.shouldDrawPlaceholder) {
        ...
        //adjust code to center regular placeholder
        NSDictionary *placeholderAttributes = @{NSFontAttributeName : self.font,
                                                NSForegroundColorAttributeName : placeholderGray};
        CGSize textSize = [self.cachedPlaceholder sizeWithAttributes:placeholderAttributes];

        CGRect placeholderFrame = CGRectMake(self.frame.size.width/2.f - textSize.width/2.f, floorf((self.frame.size.height - self.font.lineHeight) / 2.f), self.frame.size.width, self.frame.size.height);
